...
def sixcharword(string):
    nb = "".join(string.split())
    six = [nb[i:i+6] for i in range(0,len(nb),6)]
    return six

def printletter( array, num ):
    prt = ''
    for string in array:
        if num < len(string):
            prt += string[num]
...
f1 = sixcharword(found1)
f2 = sixcharword(found2)
kr = sixcharword(krypton)
...
# fnd2 = encode(f2)
# print f2[fnd2.index("ILM---")]
# print " ".join(f1)
print "".join(encode(f1))
print "".join(encode(f2))
print "".join(encode(kr))

print "FLAG: " + "".join([chr(21+65),chr(9+65),chr(22+65),chr(16+65),chr(22+65),chr(2+65)])

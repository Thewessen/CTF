...
main () {
    ...
    # removes warning 'unable to open stdin'
...
upgrade () {
    ...
    #FIXME: upgrading postgresql shows popup
    # upgrade_postgresql
    ...
    #FIXME: hangs (prob uses apt iso apt-get)
    # searchsploit --update
}
...
install_neovim () {
    ...
    apt-get install --fix-missing -y ninja-build gettext libtool libtool-bin autoconf automake cmake g++ pkg-config unzip curl doxygen
...
install_seclists () {
    ...
    apt-get install -y seclists --fix-missing
}

install_openvpn () {
    echo "install openvpn"
    apt-get install -y openvpn --fix-missing
}
